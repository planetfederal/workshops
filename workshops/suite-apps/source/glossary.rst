...
.. glossary::
    ...
    CSS
        Cascading style sheets. A system for adding visual style to :term:`HTML` documents, without embedding the style in the HTML itself.
    ...
    GeoJSON
        The `GeoJSON specification <http://geojson.org>`_ is a form of :term:`JSON` specifically for representing geographic objects, such as points, lines, and polygons.
    ...
    HTML
        Hypertext markup language. A system for tagging text to indicate document structure.

    JavaScript
        A standard programming language embedded in most web browsers. Commonly used to manipulate HTML documents in a web browser.

    JSON
        "`Javascript Object Notation <http://en.wikipedia.org/wiki/JSON>`_", a text format that is very fast to parse in Javascript virtual machines. In spatial, the extended specification for :term:`GeoJSON` is commonly used.
